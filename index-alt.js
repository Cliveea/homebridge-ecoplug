...
EcoPlug.prototype.setStatus = function (on, callback) {
	...
	this.sendMessage(message, retry_count, function (err, message) {
		if (!err) {
			console.log("Setting %s switch with ID %s to: %s", this.name, this.id, (on ? "ON" : "OFF"));
...
EcoPlug.prototype.getStatus = function (callback) {
	...
	this.sendMessage(message, retry_count, function (err, message) {
		if (!err) {
			...
			//console.log("Status of %s switch with ID %s is: %s", this.name, this.id, (status ? "ON" : "OFF"));
			console.log("%s", (status ? "ON" : "OFF"));
